...
### Tambo Playground (Showcase)

Visit `/showcase` to see a demo of Tambo's core capabilities:

- **Generative components**: Chart (bar/line/pie), SummaryCard
- **Interactable component**: Note (editable via natural language)
- **Local tool**: getSampleData (returns sample data for charts)

Try these prompts:
- "Show sales by region" → Chart with sample sales data
- "Summarize in 3 points" → SummaryCard
- "Add 'Meeting at 3pm' to the note" → Updates the interactable note
- "Change the note color to blue" → Changes note background color

### Digital Twin Notes (`/notes`)

Visit `/notes` to explore your personal notes from the Digital Twin database. Requires `DIGITAL_TWIN_DB_PATH` in `.env.local` pointing to your `digital_twin.db`.

Try these prompts:
- "Show my sentiment notes"
- "Study notes"
- "Search for planning"
- "Note 3 details"
